...
public abstract class BaseCocktail implements Cocktail{
    ...
    private void setName(String name) {
        if(name==null || name.trim().isEmpty()) throw new IllegalArgumentException(INVALID_NAME);
        this.name = name;
    }
    ...
    private void setBrand(String brand) {
        if(brand==null || brand.trim().isEmpty()) throw new IllegalArgumentException(INVALID_BRAND);
        this.brand = brand;
    }
