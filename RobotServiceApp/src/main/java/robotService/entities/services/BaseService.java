...
public abstract class BaseService implements Service{
    ...
    public void setName(String name) {
        if(name==null || name.trim().isEmpty()) throw new NullPointerException(SERVICE_NAME_CANNOT_BE_NULL_OR_EMPTY);
        this.name = name;
    }
    ...
    public String getStatistics() {
        ...
        sb.append("Robots: ");
        if(robots.isEmpty()) sb.append("none");
        else sb.append(robots.stream().map(Robot::getName).collect(Collectors.joining(" ")));
        sb.append(System.lineSeparator());
        sb.append(String.format("Supplements: %d Hardness: %d",supplements.size(),sumHardness())).append(System.lineSeparator());
