...
public class PresentImpl implements Present{
    ...
    private void setName(String name) {
        if(name==null || name.trim().isEmpty()) throw new NullPointerException(PRESENT_NAME_NULL_OR_EMPTY);
        this.name = name;
    }

    private void setEnergyRequired(int energyRequired) {
        if(energyRequired<0) throw new IllegalArgumentException(PRESENT_ENERGY_LESS_THAN_ZERO);
    ...
    public void getCrafted() {
        if(energyRequired-10<0) setEnergyRequired(0);
        else setEnergyRequired(energyRequired-10);
    }
}
